...
if __name__ == "__main__":
    for u in HHARDataset.USERS["all"]:
        for c in range(len(HHARDataset.CONTEXTS)):
            cont_data = HHARDataset(u, c)
            print(
                f"User {cont_data.user} - {cont_data.context}: ",
                "processed.",
                f"Shape: {cont_data.features.shape}",
            )
